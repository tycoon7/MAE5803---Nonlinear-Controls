...
tspan = [0 5];
% u0 = 20;
u0 = 0:5:20;        % initial speed
s0 = 0:0.1:1;       % initial s
[M1,M2] = meshgrid(u0,s0);
...
for i1 = 1:length(X0)
    [t{i1},X{i1}] = ode45(@EOM_qCarNL_robust2,tspan,X0(i1,:)',[],g,c,nu,s_d,mu_d,X0(i1,1));
    u{i1} = X{i1}(:,1);
    s{i1} = X{i1}(:,2);
    wR{i1} = (1-s{i1}).*u{i1};
%     for i2 = 1:length(t{i1})
%         [~,wR{i1}(i2)] = EOM_qCarNL_robust2(t{i1}(i2),X{i1}(i2,:)',g,c,nu,s_d,mu_d,X0(i1,1));
%     end
        plot(u{i1},wR{i1})
end
